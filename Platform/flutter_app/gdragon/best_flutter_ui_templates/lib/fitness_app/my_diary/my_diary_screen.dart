...
class _MyDiaryScreenState extends State<MyDiaryScreen>
  ...
  Future<Movements> fetchMovements() async {
    final response = await http.get('http://172.30.1.29:9090/data');

    if (response.statusCode == 200) {
      print(response.body);
      // 만약 서버로의 요청이 성공하면, JSON을 파싱합니다.
      return Movements.fromJson(json.decode(response.body));
    } else {
  ...
  Widget getMovementsUI() {
    return FutureBuilder<Movements>(
      future: movements,
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          return Text(snapshot.data.accX.toString());
...
class Movements {
  ...
  Movements({this.accX, this.accY, this.accZ, this.pose});
