    ...
    if (pathlib.Path(current_path + '/saved_data.json')).exists():
        with open(current_path + '/saved_data.json', 'w') as outfile:
            try:
                ...
                print("Failed to write saved_data.json, go ahead\n")

    if config_thingspark:
        cmd = 'GET https://api.thingspark.kr/channels/entrys\?apiKey\=6Hf8TcYw9mVaLvAm\&field1\=' + str(temp_acc_x) + '\&field2\=' + str(temp_acc_y) + '\&field3\=' + str(temp_acc_z)
        print('thinskpark curl: ', cmd)
        os.system('curl -X ' + cmd)
