      ...
      if [ $ret = 0 ] && [ $# = 5 ]; then
        ...
        if [ $? = 0 ]; then
          ...
          if [ $? = 0 ]; then
            ...
            programmertool="/Applications/STM32CubeProgrammer/STM32CubeProgrammer.app/Contents/MacOs/bin/STM32_Programmer_CLI"
            echo "fixed access path to STM32_Programmer_CLI on my macOS: $programmertool"
          fi
        fi
